...
class NavigationSimulator:
    ...
    def _load_current_map_path(self):
        ...
        if current_map in self.full_navigation['paths']:
            ...
            self.path_index = 0
            print(f"\nLoaded path for {current_map}: {len(self.current_path)} steps")
        else:
            ...
            if self.path_index < len(self.current_path):
                ...
                if self.path_index >= len(self.current_path):
                    ...
                    if self.current_map_index < len(self.map_sequence) - 1:
                        print(f"\nReached teleport in {self.current_map}")
                        
                        # Move to next map
                        self.current_map_index += 1
                        next_map = self.map_sequence[self.current_map_index]
                        
                        print(f"Teleporting to {next_map}")
                        ...
                        # Set wheelchair to start of new path
                        if self.current_path:
                            self.wheelchair_pos = self.current_path[0]
                            self.path_index = 1  # Already at position 0
                    else:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    # Start navigation with pending positions
                ...
                # Handle map switching number input
                elif self.input_mode == 'switch_map':
                    if event.key == pygame.K_RETURN:
                        # Try to switch to entered map
    ...
    def render_map(self):
        ...
        if self.current_path:
            for i, (px, py) in enumerate(self.current_path):
                if i < self.path_index:
                    # Already traversed - lighter color
                    color = (180, 220, 255)
                else:
                    # Yet to traverse
                ...
                pygame.draw.circle(self.screen, color, (sx + pixel_size//2, sy + pixel_size//2), 
                                 max(2, pixel_size//3))
        ...
        if self.pending_start and self.pending_start_map == self.current_map:
            ...
            pygame.draw.circle(self.screen, self.COLORS['start'], 
                             (sx + pixel_size//2, sy + pixel_size//2), max(5, pixel_size//2))
        ...
        if self.wheelchair_pos:
            ...
            # Draw wheelchair symbol
            radius = max(6, pixel_size//2 + 2)
            pygame.draw.circle(self.screen, self.COLORS['wheelchair'], 
                             (sx + pixel_size//2, sy + pixel_size//2), radius)
            pygame.draw.circle(self.screen, (255, 255, 255), 
                             (sx + pixel_size//2, sy + pixel_size//2), radius, 2)
    ...
    def render_status_bar(self):
        ...
        if self.map_sequence:
            ...
            # Current segment
            if self.current_path:
                progress = f"Segment {self.current_map_index + 1}/{len(self.map_sequence)} | " \
        ...
        if self.input_mode == 'start':
            input_text = ">> Click on map to set START position <<"
            input_surf = self.font_medium.render(input_text, True, (0, 255, 0))
            input_rect = input_surf.get_rect(center=(self.window_width // 2, self.canvas_height + 50))
            self.screen.blit(input_surf, input_rect)
        elif self.input_mode == 'goal':
            input_text = ">> Click on map to set GOAL position <<"
            input_surf = self.font_medium.render(input_text, True, (255, 215, 0))
            input_rect = input_surf.get_rect(center=(self.window_width // 2, self.canvas_height + 50))
            self.screen.blit(input_surf, input_rect)
        elif self.input_mode == 'switch_map':
            input_text = f">> Type map number: {self.map_input_buffer}_ (ENTER to confirm, ESC to cancel) <<"
            input_surf = self.font_medium.render(input_text, True, (100, 200, 255))
            input_rect = input_surf.get_rect(center=(self.window_width // 2, self.canvas_height + 50))
            self.screen.blit(input_surf, input_rect)
        ...
        status_surf = self.font_large.render(status, True, 
...
def run_simulator(maps_dir='maps', start_map=None, start_pos=None, 
    ...
    Args:
        ...
        start_map: Starting map ID (optional)
        start_pos: Starting position tuple (x, y) (optional)
        end_map: Ending map ID (optional)
        end_pos: Ending position tuple (x, y) (optional)
    """
    ...
    # Start navigation if all parameters provided
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        # Command line mode with full parameters
        if len(sys.argv) >= 9:
            run_simulator(
                maps_dir=sys.argv[1],
